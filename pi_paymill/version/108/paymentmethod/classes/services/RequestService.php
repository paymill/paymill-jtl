...
class RequestService
{
    ...
    /**
     * Create a paymill refund-transaction
     *
     * @param array $params
     * @param string $apiKey
     * @param string $endpoint url
     * @return array
     */
    public function createRefund($params, $apiKey, $endpoint)
    {
        try {
            $refundObject = new Services_Paymill_Refunds($apiKey, $endpoint);
            return $refundObject->create($params);
        } catch (Services_Paymill_Exception $ex) {
            Util::paymillLog('Exception thrown from paymill wrapper: ' . $ex->getMessage());
        }
    }
