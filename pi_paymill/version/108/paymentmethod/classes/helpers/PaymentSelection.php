...
class PaymentSelection
{
     ...
     */
    public static function setPaymillInfoTexts($smarty, $oPlugin, $pluginPath)
    {
        foreach (self::getPayments($smarty) as $payment) {
            if (Util::isPaymillCc($payment->cName, $oPlugin)) {
                $payment->cHinweisText[$_SESSION['cISOSprache']] .= self::getPaymillPaymentForm('cc', $payment->kZahlungsart, $pluginPath, $oPlugin);
            }

            if(Util::isPaymillElv($payment->cName, $oPlugin)) {
                $payment->cHinweisText[$_SESSION['cISOSprache']] .= self::getPaymillPaymentForm('elv', $payment->kZahlungsart, $pluginPath, $oPlugin);
            }
        }
    }
